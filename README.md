...
- **Multilingual UI** — English by default at `/`, a bilingual Chinese/English interface at `/zh`, a Chinese-only interface at `/zh-cn`, and dedicated single-language interfaces for Spanish (`/es`), Italian (`/it`), French (`/fr`), Portuguese (`/pt`), Dutch (`/nl`), German (`/de`), Japanese (`/ja`), and Korean (`/ko`), switchable from the header language menu.
...
纯中文界面：[https://padel-matchup.philosophie.ai/zh-cn](https://padel-matchup.philosophie.ai/zh-cn)
